...
import { MiniMatch } from './matches';

const includesByKey = (arr, value) => arr.map((elem) => elem.project.id).includes(value.project.id);

export default ({ matches, onUpdate }) => {
      ...
      {(provided, snapshot) => (
        ...
        >
          {rankedMatches.map((match, index) => (
            <Draggable key={match.project.id} draggableId={match.project.id} index={index}>
              {(provided, snapshot) => (
                <div
                  ref={provided.innerRef}
                ...
                >
                  <MiniMatch
                    match={match}
                    index={index}
                    onDelete={() => updateRankedMatches({ action: 'delete', data: match})} />
                </div>
              )}
            </Draggable>
          ))}
